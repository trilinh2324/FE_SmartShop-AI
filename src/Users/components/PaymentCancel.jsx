...
export default function PaymentCancel() {
  ...
  // Dùng ref tránh gọi cancel 2 lần (React StrictMode)
  ...
  // ── Gọi API cancel ngay khi trang load ──────────────────
  useEffect(() => {
    ...
    // Gọi song song: cancel + lấy thông tin bill
    Promise.all([
      // 1️⃣ Cập nhật trạng thái CANCELLED
      fetch(`${API_BASE}/api/payment/cancel/${billId}`, { method: "GET", headers })
        .then((r) => r.json())
        .then((d) => {
          if (d?.error) setCancelError(d.error);
          else setCancelDone(true);
        })
        .catch(() => setCancelError("Lỗi khi cập nhật trạng thái")),
  ...
  const goHome   = () => navigate("/users/homepage");
  const goOrders = () => navigate("/users/homepage", { state: { page: "profile" } });
  ...
  return (
    <div style={s.wrap}>
      ...
      <div style={{
        ...
        <div style={s.accent}>
          <div style={s.aBar} /><div style={s.aDot} /><div style={s.aBar} />
        </div>

        <p style={s.subtitle}>
          Đơn hàng của bạn{" "}
          <strong style={{ color: "#F0F0F0" }}>{orderCode}</strong> vẫn còn đó.
          ...
          <div style={{
            ...
            {cancelDone && (
              <span style={{ color: "#10b981", fontFamily: "'Orbitron',monospace", fontSize: 9, letterSpacing: 1.5 }}>
                ✅ ĐÃ CẬP NHẬT TRẠNG THÁI: HỦY THANH TOÁN
              </span>
            )}
            {cancelError && (
            ...
            {!cancelDone && !cancelError && (
              <span style={{ color: ORANGE, fontFamily: "'Orbitron',monospace", fontSize: 9, letterSpacing: 1.5 }}>
        ...
        {!loading && bill && (
          <div style={s.card}>
            <div style={s.cardHeader}>
              ...
              <span style={s.cancelledBadge}>❌ ĐÃ HỦY</span>
            </div>
                ...
                {items.map((item, i) => (
                  <div key={i} style={s.itemRow}>
                    ...
                    <div style={s.itemPrice}>
                      {fmt(getItemPrice(item) * getItemQty(item))}
                    </div>
                  </div>
                ))}
              </div>
        ...
        {loading && billId && (
          <div style={s.loadRow}>
            <div style={s.spin} />
            <span style={s.gray}>ĐANG XỬ LÝ...</span>
          </div>
        )}
        ...
        <div style={s.actions}>
          ...
          <button style={s.btnHome} onClick={goHome}>
            🏠 VỀ TRANG CHỦ
          </button>

          <button style={s.btnOrders} onClick={goOrders}>
            📦 XEM ĐƠN HÀNG
          </button>
        </div>
        ...
        <div style={s.support}>
          ...
          <span style={s.supportContact}>📞 84+ 339806596</span>
...
const s = {
  ...
  cancelStatusBox: { width:"100%", padding:"10px 16px", borderRadius:6, border:"1px solid", background:"rgba(0,0,0,.3)", marginBottom:16, textAlign:"center" },
  ...
  cancelledBadge:  { background:"rgba(239,68,68,.1)", border:"1px solid rgba(239,68,68,.3)", color:"#ef4444", fontFamily:"'Orbitron',monospace", fontSize:7.5, padding:"2px 9px", borderRadius:2, letterSpacing:1 },
  ...
  btnHome:          { flex:1, minWidth:130, background:"none", border:"1px solid #2a2a2a", color:"#888", fontFamily:"'Orbitron',monospace", fontSize:9.5, fontWeight:700, letterSpacing:1.2, padding:"13px 0", borderRadius:4, cursor:"pointer", transition:"border-color .2s, color .2s" },
  btnOrders:        { flex:1, minWidth:130, background:"none", border:`1px solid rgba(245,158,11,.3)`, color:ORANGE, fontFamily:"'Orbitron',monospace", fontSize:9.5, fontWeight:700, letterSpacing:1.2, padding:"13px 0", borderRadius:4, cursor:"pointer", transition:"opacity .2s" },
