...
import PaymentCancel     from "../components/PaymentCancel";   // ✅ THÊM
...
function CartPage({ setActivePage }) {
  ...
  if (!cart || cart.length === 0)
    return (
      <div style={{ textAlign: "center", padding: "100px 24px" }}>
        ...
        <div style={{ fontFamily: "'Orbitron',monospace", fontSize: 13, color: "#2a2a2a", letterSpacing: 2 }}>
          GIỎ HÀNG TRỐNG
        </div>
      </div>
    );
  ...
  return (
    <div style={{ maxWidth: 900, margin: "0 auto", padding: "50px 24px" }}>
      <h2 style={{ fontFamily: "'Bebas Neue',cursive", fontSize: 36, letterSpacing: 3, marginBottom: 28, color: "#F0F0F0" }}>
        GIỎ HÀNG
      </h2>

      {cart.map((item) => (
        <div key={item.id} style={{
          display: "flex", gap: 14, padding: "15px",
          border: "1px solid #1e1e1e", borderRadius: 4, marginBottom: 10,
          background: "#0F0F0F", alignItems: "center",
        }}>
          {item.image
            ? <img src={item.image} alt={item.name} style={{ width: 60, height: 60, objectFit: "cover", borderRadius: 4 }} />
            : <span style={{ fontSize: 38 }}>📦</span>
          }

          <div style={{ flex: 1 }}>
            <div style={{ fontFamily: "'Rajdhani',sans-serif", fontWeight: 700, fontSize: 15, color: "#F0F0F0" }}>
              {item.name}
            </div>
            <div style={{ fontFamily: "'Orbitron',monospace", fontSize: 9.5, color: "#444" }}>
              {item.description}
            </div>
          </div>

          <div style={{ fontFamily: "'Orbitron',monospace", color: "#E8000D", fontSize: 14, fontWeight: 700, whiteSpace: "nowrap" }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 7 }}>
            <button onClick={async () => {
              ...
              setCart((c) => c.map((i) => i.id === item.id ? { ...i, qty: newQty } : i));
            }} style={qB}>−</button>

            <span style={{ fontFamily: "'Orbitron',monospace", fontSize: 13, color: "#F0F0F0", width: 24, textAlign: "center" }}>
              {item.qty}
            </span>
          ...
          }} style={{ ...qB, borderColor: "#2a2a2a", color: "#555" }}>✕</button>
        </div>
      ))}

      {/* TOTAL */}
      <div style={{ textAlign: "right", marginTop: 22, padding: "20px", background: "#0F0F0F", border: "1px solid #1e1e1e", borderRadius: 4 }}>
        <div style={{ fontFamily: "'Orbitron',monospace", fontSize: 11, color: "#444", marginBottom: 6 }}>TỔNG CỘNG</div>
        <div style={{ fontFamily: "'Orbitron',monospace", fontSize: 28, color: "#E8000D", fontWeight: 900 }}>
          {formatPrice(total)}
        </div>
        <button onClick={() => setActivePage("checkout")} style={{
          marginTop: 15,
          background: "linear-gradient(135deg,#E8000D,#8B0000)",
          border: "none", color: "#fff",
          fontFamily: "'Orbitron',monospace", fontSize: 11, fontWeight: 700,
          letterSpacing: 1.5, padding: "13px 38px", borderRadius: 3,
          cursor: "pointer", boxShadow: "0 6px 22px rgba(232,0,13,.4)",
        }}>
...
const qB = {
  background: "none", border: "1px solid #E8000D", borderRadius: 3,
  color: "#E8000D", width: 28, height: 28, cursor: "pointer",
    ...
    <div style={{ background: "#0D0000", borderTop: "1px solid rgba(232,0,13,0.15)", borderBottom: "1px solid rgba(232,0,13,0.15)", padding: "22px 0", marginTop: 50 }}>
      <div style={{ maxWidth: 1400, margin: "0 auto", padding: "0 24px", display: "flex", flexWrap: "wrap", gap: 18, justifyContent: "space-between" }}>
        {[
          ...
          ["🛡️", "Bảo hành 24 tháng",  "50+ trung tâm"],
          ["💳", "Trả góp 0%",          "Lên đến 24 tháng"],
          ["📞", "Hỗ trợ 24/7",         "1800 6789 miễn phí"],
        ].map(([icon, title, desc]) => (
...
export default function HomePage() {
  ...
  // ✅ Tự động phát hiện redirect từ cổng thanh toán về
  useEffect(() => {
    const params = new URLSearchParams(window.location.search);
    const path   = window.location.pathname;

    if (params.get("cancel") === "true" || path.includes("payment-cancel") || path.includes("cancel")) {
  ...
  const handleSearch = (kw) => {
    setSearchKeyword(kw);
    if (kw.trim()) setActivePage("search");
  ...
  const renderContent = () => {
    ...
    switch (activePage) {
      case "phone":
        return <ProductSection title="ĐIỆN THOẠI" icon="📱" category="phone"  onAddCart={handleAddCart} setActivePage={setActivePage} />;
      case "ipad":
        return <ProductSection title="iPAD"       icon="📟" category="ipad"   onAddCart={handleAddCart} setActivePage={setActivePage} />;
      case "laptop":
        return <ProductSection title="LAPTOP"     icon="💻" category="laptop" onAddCart={handleAddCart} setActivePage={setActivePage} />;
      case "news":
        return <NewsPage />;
      case "about":
        return <AboutPage />;
      case "cart":
        return <CartPage cart={cart} setCart={setCart} setActivePage={setActivePage} />;
      case "checkout":
      ...
      case "payment-cancel":
        return <PaymentCancel setActivePage={setActivePage} />;
  ...
  return (
    <div style={{ background: "#070707", minHeight: "100vh" }}>
      ...
      {toast && (
        <div style={{
           position: "fixed", bottom: 110, right: 28, 
          background: "#0F0F0F", border: "1px solid #E8000D",
          borderLeft: "4px solid #E8000D", color: "#F0F0F0",
          fontFamily: "'Rajdhani',sans-serif", fontSize: 14, fontWeight: 600,
          padding: "13px 20px", borderRadius: 4,
          boxShadow: "0 8px 28px rgba(232,0,13,0.28)", zIndex: 9999,
          animation: "fadeInUp 0.3s ease both", maxWidth: 370,
        }}>
          {toast}
        </div>
